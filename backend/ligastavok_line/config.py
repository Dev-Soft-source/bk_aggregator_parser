...
class LigastavokApiConfig:
    ...
    def request_headers(self) -> dict[str, str]:
        if self.snapshot_ns == "prematch":
            referer = "https://www.ligastavok.ru/prematch"
        else:
...
LINE_BROWSER_URL = "https://www.ligastavok.ru/prematch"
DEFAULT_LINE_BROWSER_URL = LINE_BROWSER_URL
DEFAULT_LINE_POLL_INTERVAL = 10.0


def _normalize_line_browser_url(url: str | None) -> str:
    """Line hub must be https://www.ligastavok.ru/prematch — never /live."""
    raw = (url or "").strip()
    path = raw.split("?", 1)[0].rstrip("/").lower()
    if path.endswith("/prematch"):
        return raw.split("?", 1)[0].rstrip("/")
    return LINE_BROWSER_URL


def line_prune_absent_from_env() -> bool:
    """When true, delete place=line rows absent from a full prematch HTTP snapshot."""
    raw = os.getenv("LIGASTAVOK_LINE_PRUNE_ABSENT", "true").strip().lower()
    return raw in ("1", "true", "yes", "on")


def line_prune_min_fixtures_from_env() -> int:
    """Refuse prune if the prematch snapshot exported fewer fixtures than this."""
    return max(1, int(os.getenv("LIGASTAVOK_LINE_PRUNE_MIN_FIXTURES", "1") or "1"))


def line_config_from_env() -> LigastavokApiConfig:
    """Build LigastavokApiConfig for the prematch catalog."""
    from dataclasses import replace

    base = LigastavokApiConfig.from_env()
    poll_raw = (
        os.getenv("LIGASTAVOK_LINE_POLL_INTERVAL_SECONDS", "").strip()
        or os.getenv("LIGASTAVOK_POLL_INTERVAL_SECONDS", "").strip()
    )
    if poll_raw:
        poll_interval = float(poll_raw)
    else:
        poll_interval = max(float(base.poll_interval), DEFAULT_LINE_POLL_INTERVAL)

    browser_url = _normalize_line_browser_url(
        os.getenv("LIGASTAVOK_LINE_BROWSER_URL", "").strip()
        or DEFAULT_LINE_BROWSER_URL
    )
    cdp = os.getenv("LIGASTAVOK_LINE_BROWSER_CDP_URL", "").strip() or base.browser_cdp_url
