#!/usr/bin/env python3
"""Poll Liga Stavok prematch (line) and import to PostgreSQL."""
...
from ligastavok_line.poll_loop import resolve_site_name, run_poll
from ligastavok_line.config import DEFAULT_SITE_NAME, line_config_from_env
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Poll Liga Stavok prematch/line (ns=prematch → place=line) "
            "and import to PostgreSQL."
        ),
    )
    parser.add_argument(
        "--curl",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Poll interval seconds (default: LIGASTAVOK_LINE_POLL_INTERVAL_SECONDS or 10)",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="All prematch sports (default: LIGASTAVOK_LIVE_ALL_SPORTS=true)",
    )
    parser.add_argument(
        "--browser",
        action=argparse.BooleanOptionalAction,
