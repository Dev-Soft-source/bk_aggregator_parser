...
from ligastavok_line.api import LigastavokApiError, curl_has_session
...
3. In that Chrome, open https://www.ligastavok.ru/prematch and wait until prematch loads.
...
class PlaywrightCookieSession:
    ...
    def start(self) -> None:
        ...
        if self._cdp_mode and not self._page_matches_browser_url():
            logger.info(
                "CDP page is %s — navigating to line hub %s",
            ...
            try:
                ...
                logger.warning("Initial navigate to line hub failed: %s", exc)
    ...
    def _load_live_page(self, *, force_reload: bool = False) -> None:
        # CDP shortcut: cookies alone are not enough — must already be on /prematch.
