"""1xBet live main market factor mapping (same as line)."""
...
def ordered_factor_ids() -> tuple[int, ...]:
    raw = os.getenv("LXBET_LIVE_ODDS_FACTOR_IDS") or os.getenv(
    ...
    if len(ids) not in (2, 3):
        raise ValueError(
            f"LXBET_LIVE_ODDS_FACTOR_IDS must list 2 or 3 factor ids, got {raw!r}"
        )
    return tuple(ids)
...
def market_factor_maps() -> tuple[tuple[int, dict[int, int]], ...]:
    raw = os.getenv("LXBET_LIVE_MAIN_GROUP_ID") or os.getenv("LXBET_MAIN_GROUP_ID")
...
def main_group_id() -> int:
    return int(
        os.getenv("LXBET_LIVE_MAIN_GROUP_ID")
        or os.getenv("LXBET_MAIN_GROUP_ID", str(MAIN_GROUP_ID))
    )
