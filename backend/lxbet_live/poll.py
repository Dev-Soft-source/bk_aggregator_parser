#!/usr/bin/env python3
"""Poll 1xBet LiveFeed Get1x2_VZip and import odds to PostgreSQL (place=live)."""
...
from lxbet_live.api import LxbetLiveApiError, fetch_snapshot, packet_version
from lxbet_live.config import (
    DEFAULT_SITE_NAME,
    LxbetLiveConfig,
    normalize_site_name,
)
from lxbet_live.mapper import map_packet_to_changes, packet_summary
            ...
            try:
                ...
                if not coverage_ok:
                    logger.info(
                        ...
                        "fetched=%s catalog=%s) — keeping recent live rows",
                ...
                if _is_transient_network_error(exc) or isinstance(
                    exc, LxbetLiveApiError
                ):
                    logger.warning(
                        "1xBet live poll iteration %s failed: %s",
                        iteration,
                        exc,
                    )
                else:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Poll 1xBet LiveFeed Get1x2_VZip into PostgreSQL (place=live).",
    )
    parser.add_argument(
        "--site-name",
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Poll interval seconds after each cycle",
    )
    parser.add_argument(
        "--once",
        action="store_true",
    ...
    api_config = LxbetLiveConfig.from_env()
    overrides: dict = {}
    if args.site_name:
