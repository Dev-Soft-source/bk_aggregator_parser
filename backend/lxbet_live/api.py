"""HTTP client for 1xBet LiveFeed catalog + Get1x2_VZip events."""
...
class LxbetLiveApiError(Exception):
    pass


def _configure_client(cfg: LxbetLiveConfig) -> None:
    ...
    for attempt in range(1, max(1, attempts) + 1):
        try:
            ...
            if not isinstance(data, dict):
                raise LxbetLiveApiError(f"Expected JSON object from {url}")
            if data.get("Success") is False:
                raise LxbetLiveApiError(
                    f"API Success=false: {data.get('Error') or data.get('ErrorCode')}"
                )
            value = data.get("Value")
            if not isinstance(value, list):
                raise LxbetLiveApiError("Response missing Value list")
            return data
        except LxbetLiveApiError:
            raise
            ...
            if not _is_retryable(exc) or attempt >= attempts:
                raise
            logger.warning(
                "1xBet live request failed (attempt %s/%s): %s",
                attempt,
                attempts,
                exc,
            )
...
def _sport_catalog(cfg: LxbetLiveConfig) -> list[tuple[int, int, str]]:
...
def _champ_ids(cfg: LxbetLiveConfig, sport_id: int) -> list[int]:
...
def _safe_fetch(url: str, *, timeout: float) -> dict[str, Any] | None:
    try:
        ...
        logger.warning("1xBet live skip failed URL %s: %s", url, exc)
        return None
...
def _fetch_sport_events(
    ...
    try:
        ...
        logger.warning(
            "1xBet live champs list failed for sport=%s: %s", sport_id, exc
        )
        return events
...
def fetch_snapshot(config: LxbetLiveConfig | None = None) -> SnapshotResult:
    """
    Full live catalog when fetch_all_sports=true:

    1. GetSportsShortZip
    2. Per sport Get1x2_VZip?sports=ID&count=…
    3. If truncated, GetChampsZip + per-champ Get1x2_VZip
    4. Merge + optional top list
    """
    cfg = config or LxbetLiveConfig.from_env()
    _configure_client(cfg)

    if not cfg.fetch_all_sports:
        logger.info("Fetching 1xBet live snapshot: %s", cfg.snapshot_url())
        ...
        if cfg.fetch_top:
            try:
                ...
                logger.warning(
                    "1xBet live top fetch failed, using main only: %s", exc
                )
    ...
    logger.info("Fetching 1xBet live sports catalog: %s", cfg.sports_list_url())
    sports = _sport_catalog(cfg)
    expected = sum(count for _, count, _ in sports)
    logger.info(
        "1xBet live sports with events: %s (only=%s skip=%s)",
        len(sports),
    ...
    if sport_workers == 1:
        for item in sports:
            try:
                ...
                logger.warning("1xBet live sport worker failed: %s", exc)
    else:
    ...
    if cfg.fetch_top:
        try:
            logger.info("Fetching 1xBet live top: %s", cfg.top_url())
            packets.append(fetch_packet(cfg.top_url(), timeout=cfg.timeout))
        except Exception as exc:
            logger.warning("1xBet live top fetch failed: %s", exc)

    if not packets:
        raise LxbetLiveApiError("No 1xBet live packets fetched")
    merged = merge_packets(*packets)
    fetched = len(merged.get("Value") or [])
    coverage = (fetched / expected) if expected else 1.0
    logger.info(
        "1xBet live merged events: %s (catalog %s, coverage=%.0f%%)",
        fetched,
        expected,
        coverage * 100,
    )
