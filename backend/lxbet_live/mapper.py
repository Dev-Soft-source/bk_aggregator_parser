"""Map 1xBet LiveFeed Get1x2_VZip Value[] to adapter Change objects (place=live)."""
...
from lxbet_live.odds_config import (
    allowed_factor_ids,
    main_group_id,
    market_factor_maps,
    ordered_factor_ids,
...
def _timer_payload(event: dict[str, Any]) -> dict[str, Any]:
    ...
    # Live prefers SLS ("92 minutes"), then CPS ("2nd half").
    display = _as_str(sc.get("SLS")) or _as_str(sc.get("CPS")) or _as_str(sc.get("S"))
    if display:
