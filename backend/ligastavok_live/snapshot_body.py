...
# Prematch / line catalog (ns=prematch → place=line in DB).
DEFAULT_PREMATCH_ALL_BODY: dict[str, Any] = {
    "limit": 160,
    "skip": 0,
    "ns": "prematch",
    "topEvents": False,
...
def prematch_all_body(*, limit: int = 160) -> dict[str, Any]:
    body = deepcopy(DEFAULT_PREMATCH_ALL_BODY)
    body["limit"] = limit
    return body
...
def apply_prematch_all(body: dict[str, Any]) -> dict[str, Any]:
    """Strip sport filter and force prematch namespace (line catalog)."""
    patched = deepcopy(body)
    patched.pop("gameId", None)
    patched["ns"] = "prematch"
    patched.setdefault("topEvents", False)
