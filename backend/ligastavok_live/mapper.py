...
from ligastavok_live.odds_config import (
    factor_slots,
    main_market_types,
...
def _score_payload(item: dict[str, Any]) -> dict[str, Any]:
    ...
    # Fonbet-style mm:ss clock (frontend ticks seconds between polls).
    if match_minutes is not None:
        timer_seconds = match_minutes * 60
        timer_display = f"{match_minutes}:{0:02d}"
    else:
        timer_seconds = None
        timer_display = event.get("statusTranslated") or (
...
def _odds_outcomes(item: dict[str, Any]) -> list[OddsOutcome]:
    ...
    for outcome in outcomes_raw.values():
        ...
        if value is None or fac_id is None:
            continue
        slot_idx = keys_order.get(outcome_key, 99)
        ...
        picked.append(
            (
                slot_idx,
                OddsOutcome(
                    # UI / odds_lines.factor_id is INTEGER — use Fonbet-compatible slots.
                    factor_id=slots[slot_idx],
                    odds=float(value),
                    line_param_raw=int(fac_id),
    ...
    return [o for _, o in picked[: len(slots)]]
