"""Tests for live absent-match prune gating."""
...
from ligastavok_live.poll_loop import should_prune_live_absent
...
class ShouldPruneLiveAbsentTests(unittest.TestCase):
    def test_prunes_full_http_live_snapshot(self) -> None:
        changes = [_change(match_id=10), _change(match_id=20)]
        do_prune, ids = should_prune_live_absent(
            changes,
            snapshot_ns="live",
            prune_enabled=True,
            min_fixtures=1,
    ...
    def test_skips_when_disabled(self) -> None:
        ...
        do_prune, _ = should_prune_live_absent(
            changes,
            snapshot_ns="live",
            prune_enabled=False,
        )
    ...
    def test_skips_prematch_ns(self) -> None:
        changes = [_change(match_id=10)]
        do_prune, _ = should_prune_live_absent(
            changes,
