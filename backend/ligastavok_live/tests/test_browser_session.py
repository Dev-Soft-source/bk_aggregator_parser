...
from ligastavok_live.browser_session import build_cookie_header
...
class BuildCookieHeaderTests(unittest.TestCase):
    ...
    def test_navigate_live_goto_from_homepage(self) -> None:
        from unittest.mock import MagicMock
    ...
    def test_cdp_load_skips_only_when_already_on_live(self) -> None:
        ...
        page.url = "https://www.ligastavok.ru/live"
        session = PlaywrightCookieSession.__new__(PlaywrightCookieSession)
        session._cdp_mode = True
        session._page = page
        session._config = MagicMock(
            browser_url="https://www.ligastavok.ru/live",
            browser_timeout_seconds=5,
            promo_auto_click=False,
        )
        session._context = MagicMock()
        session._promo_dismissed = False

        with patch(
            "ligastavok_live.browser_session.build_cookie_header",
            return_value="qrator_jsid2=abc",
        ):
            session._load_live_page()
        page.goto.assert_not_called()
        page.reload.assert_not_called()

    def test_cdp_load_navigates_from_homepage_even_with_cookies(self) -> None:
        from unittest.mock import MagicMock, patch
        ...
        page = MagicMock()
        page.url = "https://www.ligastavok.ru/"
        # expect_response is a context manager
        cm = MagicMock()
