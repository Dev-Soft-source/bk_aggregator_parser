...
from ligastavok_live.api import curl_has_session, save_snapshot
from ligastavok_live.config import (
    LigastavokApiConfig,
    live_prune_absent_from_env,
    live_prune_min_fixtures_from_env,
)
from ligastavok_live.mapper import extract_events
...
def should_prune_live_absent(
    ...
    *,
    ...
    """
    Prune only on full live HTTP snapshots — never on WebSocket deltas.

    WS batches include from_version and only touch a few events; pruning on them
    would delete the rest of the live catalog.
    """
    fixture_ids = active_fixture_ids(changes)
    if not prune_enabled or snapshot_ns != "live":
...
def run_poll(
    *,
    ...
    if api_config.use_playwright:
        from ligastavok_live.browser_session import PlaywrightCookieSession
    ...
    try:
        ...
        prune_enabled = (
            live_prune_absent_from_env() if api_config.snapshot_ns == "live" else False
        )
        prune_min = live_prune_min_fixtures_from_env()
        if api_config.snapshot_ns == "live":
            print(
        ...
        with connect(db_config) as conn:
            for packet, changes, summary in adapter.stream_poll_changes(
                ...
                try:
                    do_prune, active_ids = should_prune_live_absent(
                        changes,
                    ...
                    snapshot_id, counts = apply_changes(
                        ...
                        prune_place="live" if do_prune else None,
                    )
        ...
        adapter.close()
        from ligastavok_live.api import close_http_session
